...
export async function cancelChildRunsOf(parentRunId: string): Promise<number> {
  ...
  for (const child of children) {
    cancelled += await cancelChildRunsOf(child.id); // nested fan-out (future) first
    const r = await cancelOneRun(child.id);
...
export async function cancelRunTree(
  ...
  const childrenCancelled = await cancelChildRunsOf(runId);
  const self = await cancelOneRun(runId);
